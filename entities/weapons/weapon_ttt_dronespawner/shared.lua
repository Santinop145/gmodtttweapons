...
function SWEP:PrimaryAttack()
   if(SERVER) then
      if(not self.DroneSpawned) then
         PrintMessage(HUD_PRINTCENTER, "A drone has been spawned!")
         self.DroneSpawned = true
         local drone = ents.Create("dronesrewrite_bomb")
         drone:SetPos(self:GetOwner():GetEyeTrace().HitPos)
         drone.Speed = 2000
         drone.UpSpeed = 5000
         drone.Fuel = 1000
         drone.MaxFuel = 1000
         drone.FuelReduction = 0.1
